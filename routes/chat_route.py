...
chat_model = chat_ns.model("ChatRequest", {
    ...
    "message": fields.String(required=True, description="사용자 발화")
})

@chat_ns.route("")
class Chat(Resource):
    @chat_ns.doc(
        description="챗봇 메인 API",
        responses={
            200: "챗봇이 성공적으로 응답함 -> data 반환",
            400: "JSON Body가 없거나 올바르지 않은 data 형식",
            500: "서버 내부 오류"
        }
    )
